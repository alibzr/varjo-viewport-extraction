...
{
    int32_t index;
    varjo_AcquireSwapChainImage(m_colorSwapChain, &index);
    if (varjo_GetError(m_session) == varjo_NoError) {
        const varjo_Texture swapchainTexture = varjo_GetSwapChainImage(m_colorSwapChain, index);
        ...
        for (int i = 0; i < 2; i++) {
            ...
            copyBox.top = view.viewport.y + 1880;
            copyBox.right = view.viewport.x + view.viewport.width;
            copyBox.bottom = view.viewport.y + view.viewport.height + 1880;
