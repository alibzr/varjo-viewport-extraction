...
void StreamingApplication::update()
{
    ...
    // Calculate received frames
    static size_t frames = 0;
    ++frames;

    // Calculate dropped frames
    static int64_t lastFrame = m_frameNumber - 1;
    static int64_t droppedFrames = 0;
    droppedFrames += m_frameNumber - (lastFrame + 1);
    lastFrame = m_frameNumber;

    // Log stats every 10 seconds
    const auto now = std::chrono::steady_clock::now();
    static auto lastPrint = now;
    const auto elapsedMs = std::chrono::duration_cast<std::chrono::milliseconds>(now - lastPrint).count();
    if (elapsedMs >= 10000) {
        LOG_INFO("Frame %lli FPS %.1f Dropped %lli frames", lastFrame, frames * 1000.0 / elapsedMs, droppedFrames);
        lastPrint = now;
        frames = 0;
        droppedFrames = 0;
    }
